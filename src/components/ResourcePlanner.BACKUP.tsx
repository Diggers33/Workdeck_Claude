 ...
 * - /queries/tasks/user/{userId} - User's task assignments (for allocation calculation)
...
export function ResourcePlanner() {
  ...
  const loadResourceData = async () => {
    try {
      ...
      // Only fetch summary data on initial load - gantt-plus will be fetched on-demand when rows expand
      ...
      console.log(`[ResourcePlanner] Initial API calls took ${((performance.now() - loadStartTime) / 1000).toFixed(2)}s`);
      ...
      console.log(`[ResourcePlanner] Loaded ${projects.length} projects from API`);
      ...
      // Build team members WITHOUT assignments initially - assignments will be loaded on-demand
      ...
      console.log(`[ResourcePlanner] Created ${membersWithTasks.length} team members`);
      ...
      // Start background loading of all gantt-plus data
  ...
  // Load all gantt-plus data in background (non-blocking)
  // Collects all data first, then does a SINGLE state update to avoid re-render jank
  const loadAllGanttDataInBackground = async (projects: any[], initialMembers: TeamMember[]) => {
    if (projects.length === 0) return;

    setIsLoadingGanttData(true);
    setGanttLoadProgress({ loaded: 0, total: projects.length });
    console.log(`[ResourcePlanner] Starting background load of ${projects.length} projects...`);

    const startTime = performance.now();
    let loadedCount = 0;

    // Collect all data first, then do single state update
    const allGanttCache: Record<string, any[]> = {};
    ...
    for (let i = 0; i < projects.length; i += BATCH_SIZE) {
      ...
      const batchPromises = batch.map(async (project) => {
        try {
          ...
          console.warn(`[ResourcePlanner] Failed to fetch gantt-plus for ${project.name}:`, e);
      ...
      // Collect data (don't update state yet)
      batchResults.forEach(({ project, tasks }) => {
        // Collect for cache
        allGanttCache[project.id] = tasks;

        // Build assignments from this project's tasks
      ...
      // Only update progress (lightweight)
      setGanttLoadProgress({ loaded: loadedCount, total: projects.length });
    }

    // NOW do a single state update with all collected data
    console.log(`[ResourcePlanner] Applying ${allAssignments.length} assignments in single update...`);

    // Update cache in one go
    setProjectGanttCache(prev => ({ ...prev, ...allGanttCache }));

    // Update team members in one go
    ...
    console.log(`[ResourcePlanner] Background load complete: ${loadedCount}/${projects.length} projects in ${((endTime - startTime) / 1000).toFixed(2)}s`);
  ...
  const saveAllocations = useCallback(async () => {
    ...
    try {
      ...
      // Group allocations by task AND person - each user needs their own API call
  ...
  const getTaskBarStyle = (assignment: Assignment, periods: { start: Date; end: Date }[]) => {
  ...
  // Render timeline row
  const renderTimelineRow = (member: TeamMember, isLast: boolean) => (
  ...
  return (
    <div className="flex flex-col h-full" style={{ backgroundColor: '#F9FAFB' }}>
      {/* Header */}
      <div style={{ backgroundColor: 'white', borderBottom: '1px solid #E5E7EB', padding: '16px 24px', flexShrink: 0 }}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: '600', color: '#111827', marginBottom: '4px' }}>Resource Planner</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <div style={{ display: 'flex', backgroundColor: '#F3F4F6', borderRadius: '6px', padding: '3px' }}>
              ...
              <button
                onClick={() => setDisplayUnit('personMonths')}
                style={{
                  padding: '5px 10px',
                  borderRadius: '4px',
        ...
        <div className="grid grid-cols-6 gap-3 mb-4">
          ...
          <div
            style={{ padding: '12px 16px', backgroundColor: allocationFilter === 'over' ? '#FEE2E2' : '#FEF2F2', borderRadius: '8px', border: `1px solid ${allocationFilter === 'over' ? '#EF4444' : '#FEE2E2'}`, cursor: 'pointer' }}
            onClick={() => setAllocationFilter(allocationFilter === 'over' ? 'all' : 'over')}
          >
            <div className="flex items-center gap-2 mb-1">
              <AlertTriangle className="w-4 h-4" style={{ color: '#DC2626' }} />
              <span style={{ fontSize: '12px', color: '#991B1B' }}>Over</span>
            </div>
            <div style={{ fontSize: '20px', fontWeight: '600', color: '#DC2626' }}>{stats.overAllocated}</div>
          </div>

          <div
            style={{ padding: '12px 16px', backgroundColor: allocationFilter === 'under' ? '#FDE68A' : '#FEF3C7', borderRadius: '8px', border: `1px solid ${allocationFilter === 'under' ? '#D97706' : '#FDE68A'}`, cursor: 'pointer' }}
            onClick={() => setAllocationFilter(allocationFilter === 'under' ? 'all' : 'under')}
          >
            <div className="flex items-center gap-2 mb-1">
              <Clock className="w-4 h-4" style={{ color: '#D97706' }} />
              <span style={{ fontSize: '12px', color: '#92400E' }}>Under</span>
            </div>
            <div style={{ fontSize: '20px', fontWeight: '600', color: '#D97706' }}>{stats.underAllocated}</div>
          </div>

          <div
            style={{ padding: '12px 16px', backgroundColor: allocationFilter === 'optimal' ? '#BBF7D0' : '#F0FDF4', borderRadius: '8px', border: `1px solid ${allocationFilter === 'optimal' ? '#16A34A' : '#BBF7D0'}`, cursor: 'pointer' }}
            onClick={() => setAllocationFilter(allocationFilter === 'optimal' ? 'all' : 'optimal')}
          >
            <div className="flex items-center gap-2 mb-1">
              <CheckCircle className="w-4 h-4" style={{ color: '#16A34A' }} />
              <span style={{ fontSize: '12px', color: '#166534' }}>Optimal</span>
            </div>
            <div style={{ fontSize: '20px', fontWeight: '600', color: '#16A34A' }}>{stats.optimal}</div>
          </div>

          <div
            style={{ padding: '12px 16px', backgroundColor: allocationFilter === 'conflicts' ? '#FEE2E2' : '#FFF7ED', borderRadius: '8px', border: `1px solid ${allocationFilter === 'conflicts' ? '#EF4444' : '#FDBA74'}`, cursor: 'pointer' }}
            onClick={() => setAllocationFilter(allocationFilter === 'conflicts' ? 'all' : 'conflicts')}
          >
            <div className="flex items-center gap-2 mb-1">
              <AlertCircle className="w-4 h-4" style={{ color: '#EA580C' }} />
              <span style={{ fontSize: '12px', color: '#9A3412' }}>Conflicts</span>
            </div>
            <div style={{ fontSize: '20px', fontWeight: '600', color: '#EA580C' }}>{stats.withConflicts}</div>
          </div>
          ...
          <div style={{ padding: '12px 16px', backgroundColor: '#EFF6FF', borderRadius: '8px', border: '1px solid #BFDBFE' }}>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span style={{ fontSize: '12px', color: '#1E40AF' }}>Total PM</span>
            </div>
            <div style={{ fontSize: '20px', fontWeight: '600', color: '#2563EB' }}>{stats.totalPM.toFixed(2)}</div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showPersonDropdown && (
                <div
                  ...
                  onClick={() => { setShowPersonDropdown(false); setPersonSearchQuery(''); }}
                />
              )}
            </div>
            ...
            {(selectedDepartment !== 'all' || selectedProject !== 'all' || selectedPeople.size > 0 || selectedSkill !== 'all' || allocationFilter !== 'all') && (
              <button
                onClick={() => { setSelectedDepartment('all'); setSelectedProject('all'); setSelectedPeople(new Set()); setSelectedSkill('all'); setAllocationFilter('all'); }}
          ...
          <div className="flex items-center gap-3">
            ...
            {viewMode === 'timeline' && (
              <>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => navigateDate('prev')} style={{ padding: '6px', borderRadius: '6px', border: '1px solid #E5E7EB', backgroundColor: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                  ...
                  <button onClick={() => navigateDate('next')} style={{ padding: '6px', borderRadius: '6px', border: '1px solid #E5E7EB', backgroundColor: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '20px 24px' }}>
        {isLoading && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '64px', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #E5E7EB' }}>
            <Loader2 className="w-8 h-8 animate-spin" style={{ color: '#0066FF', marginBottom: '16px' }} />
            <div style={{ fontSize: '14px', color: '#6B7280' }}>Loading resource data...</div>
          </div>
        )}
        ...
        {/* Timeline View */}
        {!isLoading && !error && filteredMembers.length > 0 && viewMode === 'timeline' && (
        ...
        {/* Forecast View */}
        {!isLoading && !error && teamMembers.length > 0 && viewMode === 'forecast' && (
        ...
        {/* Skills View */}
        {!isLoading && !error && filteredMembers.length > 0 && viewMode === 'skills' && (
        ...
        {!isLoading && !error && teamMembers.length > 0 && viewMode === 'grid' && (
          <div style={{ backgroundColor: 'white', borderRadius: '8px', border: '1px solid #E5E7EB', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  <div style={{ display: 'flex', backgroundColor: '#F3F4F6', borderRadius: '6px', padding: '2px' }}>
                    <button
                      onClick={() => setGridHierarchy('person')}
                      style={{
                        padding: '4px 10px',
                        borderRadius: '4px',
                        border: 'none',
                    ...
                    <button
                      onClick={() => setGridHierarchy('project')}
                      style={{
                        padding: '4px 10px',
                        borderRadius: '4px',
                        border: 'none',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  <div style={{ display: 'flex', backgroundColor: '#F3F4F6', borderRadius: '6px', padding: '2px' }}>
                    <button
                      onClick={() => setGridGranularity('monthly')}
                      style={{
                        padding: '4px 10px',
                        borderRadius: '4px',
                        border: 'none',
                    ...
                    <button
                      onClick={() => setGridGranularity('weekly')}
                      style={{
                        padding: '4px 10px',
                        borderRadius: '4px',
                        border: 'none',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  <select
                    ...
                    onChange={(e) => setGridMonthsToShow(parseInt(e.target.value))}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                {/* Date Navigation */}
                <button
                  onClick={() => setGridStartDate(prev => new Date(prev.getFullYear(), prev.getMonth() - 1, 1))}
                ...
                <button
                  onClick={() => setGridStartDate(new Date(new Date().getFullYear(), new Date().getMonth(), 1))}
                            ...
                            {gridPeriods.map((period, idx) => {
                              ...
                              return (
                                <td key={period.key} style={{ padding: '2px', textAlign: 'center', borderBottom: '1px solid #F3F4F6', borderLeft: idx === 0 || (gridGranularity === 'monthly' && period.startDate.getMonth() === 0) ? '2px solid #D1D5DB' : '1px solid #F3F4F6', backgroundColor: period.startDate.getMonth() % 2 === 0 ? (expandedRows.has(child.id) ? '#F0F9FF' : '#FAFAFA') : 'rgba(0,0,0,0.02)' }}>
                                  {/* Read-only aggregate cell with MEMOIZED no-op handler */}
                                  {(() => {
      ...
      {/* Modals */}
      {selectedTask && <TaskDetailModal assignment={selectedTask.assignment} member={selectedTask.member} onClose={() => setSelectedTask(null)} />}
      {tooltip && <Tooltip assignment={tooltip.assignment} position={tooltip.position} displayUnit={displayUnit} />}
      {editingCapacity && <CapacityEditPopover member={editingCapacity.member} position={editingCapacity.position} onSave={updateMemberCapacity} onClose={() => setEditingCapacity(null)} />}
      {editingLeave && <LeaveModal member={editingLeave} onClose={() => setEditingLeave(null)} onSave={updateMemberLeave} />}
    </div>
  );
}
